...
if(data_location == "server"){
  ...
  outFolder <- "C:\\Users\\nicol\\OneDrive\\Desktop\\scrnaseq_gastruloids\\results\\integration\\pijuan_BraGFP_WT\\"
} else if(data_location == "ext_drive"){
  folder.RData <- "F:\\scrnaseq_gastruloids\\data\\"
  outFolder <- "F:\\scrnaseq_gastruloids\\results\\integration\\pijuan_BraGFP_WT\\"
}
load(paste0(folder.RData,"anlas_data.RData"))
load(paste0(folder.RData,"anlas_WT_data.RData"))
...
# filter anlas data to only good cells and genes
anlas_WT <- anlas_WT[rowSums(anlas_WT)>10,]
anlas_WT <- anlas_WT[grep('mt-',rownames(anlas_WT),invert=T),]

anlasBra <- anlas[,anlas$merge.ident=='48h']
anlasBra <- anlasBra[rowSums(anlasBra)>10,]
anlasBra <- anlasBra[grep('mt-',rownames(anlasBra),invert=T),]
anlasBra$merge.ident <- paste0("BraGFP_",anlasBra$merge.ident)

anlas <- merge(anlasBra, y=anlas_WT, add.cell.ids = c("BraGFP", "WT"))
remove(anlas_WT, anlasBra)
gc()
...
for(i in 1:length(meta_anlas$celltype.anlas)){
  ...
  if(grepl("Early ", old)){
    ...
    c[i] <- 'G Neural'
  } else if(grepl("Endodermal", old)){
    c[i] <- 'G Endoderm'
  } else if(grepl("Mesendodermal", old)){
    c[i] <- 'G Mesoderm'
  } else{
    c[i] <- "G_WT"
  }
